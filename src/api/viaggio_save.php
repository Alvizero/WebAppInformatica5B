...
if (!$dest || $lat === false || $lng === false || !$inizio || !$fine || $fine < $inizio) {
    header('Location: ./../dashboard/dashboard.php?error=dati_non_validi');
    exit;
}

if ($id) {
    $stmt = $pdo->prepare(
        'UPDATE viaggi SET destinazione=:dest, latitudine=:lat, longitudine=:lng,
                           data_inizio=:inizio, data_fine=:fine
         WHERE id=:id AND user_id=:uid'
    );
    $stmt->execute([
        ':dest' => $dest, ':lat' => $lat, ':lng' => $lng,
